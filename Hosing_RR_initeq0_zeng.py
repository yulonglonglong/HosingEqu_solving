...
def solveBORR(Xb0, gamma_init, s_max, xi_max, ns, nxi):
    ...
    plt.figure()
    plt.plot(s, Xb[:, 1], 'g-', label='Xb-s')
    plt.title(label='k_p xi={}'.format(xi[1]))
    plt.ylabel('Xc')
    plt.xlabel('s')  
    plt.figure()
    plt.plot(xi, Xc[-2, :], 'g-', label='Xc-s')
    plt.plot(xi, np.sin(np.power(1./2., 1/2)*xi)+Xb[-2, :], 'r-', label='Xc-s_theory')
    plt.title(label='k_p xi={}'.format(s[-1]))
    plt.ylabel('Xc')
    plt.xlabel('s')
    plt.legend()   
...
if '__main__'==__name__:
    ...
    plt.subplot(223)
    xi_ind = -1
    plt.plot(s, Xb[:, xi_ind], label = 'Numerical')
